...
def load_data(region_paths, bird_classes):
    ...
    data_loaders = {
        'train': DataLoader(datasets['region3'], batch_size=32, shuffle=True, drop_last=True),
        'test_region2': DataLoader(datasets['region2'], batch_size=32, shuffle=False, drop_last=False),
        'test_region1': DataLoader(datasets['region1'], batch_size=32, shuffle=False, drop_last=False)
    }
    return data_loaders
...
def evaluate_model(model, test_loader, region_name=None):
    ...
    report = classification_report(all_labels, all_preds, target_names=bird_classes, digits=4)
    print(report)
...
def train_model(model, criterion, optimizer, train_loader, val_loader_1=None, val_loader_2=None, num_epochs=40):
    ...
    for epoch in range(num_epochs):
        ...
        if f1_sum > best_f1_sum:
            ...
            torch.save(best_model_wts, "multi_branch_cnn_g_m_wi_train1_d.pth")
    ...
    if best_model_wts is not None:
        ...
        print(f"Training finished. Best model saved as 'multi_branch_cnn_g_m_wi_train1_d.pth' with F1_sum={best_f1_sum:.4f}")
region_paths = {
...
if __name__ == '__main__':
    ...
    train_model(model, criterion, optimizer,
                ...
                val_loader_1=data_loaders['test_region2'],
                # val_loader_2=data_loaders['test_region3'],
                num_epochs=40)

    # 加载最佳模型进行最终测试
    print("\n=== Loading Best Model for Final Testing ===")
    best_model_path = "multi_branch_cnn_g_m_wi_train1_d.pth"
