...
def train_model(model, criterion, optimizer, train_loader, val_loader=None, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        if f1_r2 > best_f1:
            ...
            torch.save(best_model_wts, "best_s1s2_mel_wo_atten_f1.pth")
    ...
    if best_model_wts is not None:
        ...
        print(f"Training finished. Best model saved as 'best_s1s2_mel_wo_atten_f1.pth' with F1={best_f1:.4f}")

# 主程序
if __name__ == '__main__':
    model = MultiBranchCNN_M_WO(num_classes=len(bird_classes)).to(device)
    ...
    region_paths = {
        'region1': {'mel': os.path.join(mel_path, 'S1')},
        'region2': {'mel': os.path.join(mel_path, 'S2')}
    }
    ...
    best_model_path = "best_s1s2_mel_wo_atten_f1.pth"
